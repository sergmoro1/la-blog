<div class="field">
  ...
  <div class="control">
    <div class="select">
      <select x-cloak id="select" wire:model="{{ $field }}">
        @foreach ($options as $option)
          <option value="{{ $option->id }}">{{ $option->name }}</option>
        @endforeach
      </select>

      <div x-data="dropdown()" x-init="loadOptions()" class="flex flex-col field">
        <input name="values" type="hidden" x-bind:value="selectedValues()">
        <div class="control">
          <div class="flex flex-col items-center relative">
            <div x-on:click="open" class="w-full">
              <div class="p-1 flex border border-gray-400 bg-white rounded">
              ...
              <div x-show.transition.origin.top="isOpen()" class="absolute shadow top-100 bg-white z-40 w-full left-0 rounded max-h-select" x-on:click.away="close">
                <div class="flex flex-col w-full overflow-y-auto h-64">
                  <template x-for="(option,index) in options" :key="option" class="overflow-auto">
                    <div class="cursor-pointer w-full border-gray-400 rounded-t border-b hover:bg-gray-100" @click="select(index,$event)">
